...
const SidebarWapper = styled.div<{ isSidebarEnabled: boolean }>`
  min-height: 100%;
  min-width: 250px;
  max-width: 250px;
  user-select: none;
  ...
  @media (max-width: ${screenSizes.tabletScreenSize}px) {
    ...
    ${(p): string => (p.isSidebarEnabled ? "" : "margin-left: -100%;")}
  }
`;
...
const Sidebar: FC = () => {
  ...
  const randomVideoURL = "watch/" + randomVideo?.id ?? "";
  ...
  return (
    <SidebarWapper isSidebarEnabled={isSidebarEnabled}>
      ...
      <SidebarContent>
        <Logo />
        <SideBarButton title={"Home"} icon={"bx-home-alt-2 bx-sm"} url={""} selectedURL={selectedURL} />

        <SideBarButton title={"Favorites"} icon={"bx-heart bx-sm"} url={"favorites"} selectedURL={selectedURL} />

        <SideBarButton title={"All Videos"} icon={"bx-list-ul bx-sm"} url={"all"} selectedURL={selectedURL} />

        <SideBarButton title={"Random"} icon={"bx-dice-6 bx-sm"} url={randomVideoURL} selectedURL={selectedURL} />

        {categories.length ? (
          <SideBarButton title={"Library"} icon={"bx-folder bx-sm"} url={libraryURL} selectedURL={selectedURL} />
        ) : (
          <></>
        )}
